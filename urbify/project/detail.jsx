// detail.jsx — Urbify property detail
              ...
              <div style={{marginTop:24, padding:'18px 18px', background:'var(--surface-sunken)', borderRadius:'var(--r-md)'}}>
                ...
                <div style={{display:'flex', flexDirection:'column', gap:6, fontSize:13}}>
                  <div style={{display:'flex', justifyContent:'space-between', color:'var(--text-muted)'}}>
                    <span>₹{listing.rentK}k ÷ 30 × 7.5 days</span>
...
function MiniMap({label}) {
  return (
    <div style={{height:'100%', position:'relative', background:'#E8F0EE'}}>
      <svg viewBox="0 0 400 260" style={{width:'100%', height:'100%'}}>
        <defs>
          <pattern id="grid2" width="30" height="30" patternUnits="userSpaceOnUse">
            <path d="M30 0 L0 0 L0 30" fill="none" stroke="#D0DEDA" strokeWidth=".5"/>
          </pattern>
        </defs>
        <rect width="400" height="260" fill="#E8F0EE"/>
        <rect width="400" height="260" fill="url(#grid2)"/>
        <path d="M0 130 Q 100 120, 200 140 T 400 150" stroke="#fff" strokeWidth="12" fill="none"/>
        <path d="M150 0 L 180 80 L 170 200 L 220 260" stroke="#fff" strokeWidth="8" fill="none"/>
        <polygon points="80,60 260,40 280,180 100,200" fill="#0D7C66" opacity=".22" stroke="#0D7C66" strokeWidth="1.5"/>
        <text x="180" y="110" textAnchor="middle" fontFamily="Plus Jakarta Sans" fontSize="14" fontWeight="700" fill="#074D40">{label.toUpperCase()}</text>
        <text x="180" y="128" textAnchor="middle" fontFamily="Inter" fontSize="10" fill="#074D40" opacity=".7">exact pin hidden</text>
      </svg>
    </div>
  );
}
