// compare-page.jsx — Compare 2-3 listings side-by-side

function ComparePage({nav, savedIds, onSave, onUnlock}) {
  const [pickerOpen, setPickerOpen] = useState(false);
  const [picked, setPicked] = useState([LISTINGS[0].id, LISTINGS[3].id, LISTINGS[5].id]);
    ...
    <div>
      ...
      <section style={{padding:'24px 28px 80px', maxWidth:1440, margin:'0 auto'}}>
        <div className="card" style={{padding:0, overflow:'hidden'}}>
          ...
          <Row label="Configuration" tpl={tplCols}>
            {items.map((l, i) => cell(`${l.bhk} BHK`, null))}
          </Row>
          <Row label="Carpet area" tpl={tplCols}>
            {items.map((l, i) => cell(`${fmt(l.area)} sq ft`, 'max', items.map(x=>x.area)))}
          </Row>
          <Row label="Monthly rent" tpl={tplCols}>
            {items.map((l, i) => cell(`₹${l.rentK}k`, 'min', items.map(x=>x.rentK), `per sq ft: ₹${Math.round(l.rentK*1000/l.area)}`))}
          </Row>
          <Row label="Deposit" tpl={tplCols}>
            {items.map((l, i) => cell(`₹${l.deposit}k`, 'min', items.map(x=>x.deposit)))}
          </Row>
          <Row label="Unlock fee" tpl={tplCols} last>
            {items.map((l, i) => cell(`₹${fmt(l.feeGST)}`, 'min', items.map(x=>x.feeGST), 'incl. 18% GST'))}
          </Row>

          {/* Group: Property */}
          <GroupHeader>Property</GroupHeader>
          <Row label="Floor" tpl={tplCols}>{items.map((l,i)=>cell(`${l.floor} of ${l.total}`, null))}</Row>
          <Row label="Facing" tpl={tplCols}>{items.map((l,i)=>cell(l.facing, null))}</Row>
          <Row label="Furnishing" tpl={tplCols}>{items.map((l,i)=>cell(l.furnishing, null))}</Row>
          <Row label="Age" tpl={tplCols}>{items.map((l,i)=>cell(l.age, null))}</Row>
          <Row label="Available from" tpl={tplCols} last>{items.map((l,i)=>cell(l.available, null))}</Row>
          ...
          <Row label="Listing party" tpl={tplCols}>
            {items.map((l, i)=>(
          ...
          <Row label="Posted" tpl={tplCols} last>{items.map((l,i)=>cell(l.posted, null))}</Row>
...
function Row({label, children, tpl, last}) {
  return (
